...
- [ ] Gate: 2025-26 gated replay, three seeds a side, tolerance 5 and hits ≤ +3 — **which sees §4.5 and no-regresses the other five**, because `backtest.py` reaches only `free_hit_gain`; both sides at the shipped defaults with `config.toml` identical between them, since the backtest refits and `[model] xg_per_shot` is a live lever there; plus §4.4's captain-support check (drop ≤ 10 points, S1's failure signature by name)
...
- Suite: 3588 Python + 703 frontend (G1, implementer-measured)
