...
(`src/gaffer/web/static/`) is regenerated by `npm run build` as the final task. It is
**gitignored by design** and never committed — the wheel force-includes it via hatch
`artifacts` (`pyproject.toml:36-37`); the plan's original "commit the bundle" step was
a wrong assumption, discovered and skipped during Group C.
...
Cycle closed 2026-08-30. Three implementer groups (Tasks 1–18) + one fix round, all on
Opus; orchestrator ran every gate. Adversarial review: **1 blocker + 3 importants + 5
nits**; all fixed except two recorded residuals. Suites: **1636 Python** (from 1619),
**302 frontend + 1 skip** (from 251), `tsc -b` clean; protected files zero diffs.

The blocker falsified D4's "all colour consumption is var-backed": Tailwind v4 bakes
opacity-modifier utilities (`border-sage/40` et al.) to literal dark hex+alpha in the
compiled CSS — invisible at source level, found only by scanning the built bundle.
Fixed with `--color-{sage,rust,info}-soft` tokens in both palettes, plus a built-CSS
guard test (`built-theme.test.ts`, mutation-tested) that asserts the light block sits
unlayered in the compiled artifact and no dark hex is ever re-baked. Second-order
hazard discovered during the fix: **Tailwind's content scanner reads comments — prose
that spells a utility class name regenerates that class in the bundle.**

Other review fixes: light muted `#686f7d` (≥4.5:1 on card *and* base — nav links sit
on base), faint `#7d848e` (deliberate sub-AA tier, parity with dark, floor-pinned at
3.5:1), segmented toggle label-only + `min-w-0` (176px sidebar budget), `/api/pens`
422s (not 500s) on corrupt artifacts with a non-silent frontend error line,
`color-scheme` declarations, README lines for `--fast` and the theme toggle.

### Gate evidence (convention 4)

```
G1  time uv run gaffer advise --fast → completed in 78s wall (vs ~6 min swept),
    no "Scenarios:" line; picks/captain rendered normally
G2  GET /api/pens → 200, gw1 block {instrument xg_gap, covered_rows 256,
    team_games 20, taker_hit_rate 1.0}; corrupt/missing artifact → 422 (unit)
G3  frontend 302 passed + 1 skip (52 files); tsc -b clean; npm run build 985ms;
    built-CSS guard passes against the fresh bundle (light block unlayered at
    depth 0, zero baked dark border hexes)
G4  uv run pytest -q -p no:randomly → 1636 passed (2:24); protected audit
    (incl. web/jobs.py + routers/jobs.py) → empty; server restarted: GET / 200,
    boot script present in served index.html, /api/pens live
    Final light contrast: text 17.07/15.64, secondary 9.95/9.12,
    muted 5.05/4.63 (card/base), faint 3.77 on card
```

### Residuals (recorded, not fixed)

- `useTheme` state is per-instance with no `storage`-event listener: a second browser
  tab keeps its theme until reload. Only one `ThemeToggle` mounts per breakpoint
  today, so nothing desyncs in-page.
- The `/api/quality` vs `/api/pens` empty-state copy uses slightly different backtick
  conventions in older messages ("run `gaffer evaluate` first" style aligned for pens
  only).
