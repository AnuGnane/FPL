...
def fit_calibration(df: pd.DataFrame, tg: pd.DataFrame,
    ...
    The newest season present is held out: every component is refit on the
    seasons before it, the holdout season is predicted and assembled, and the
    calibration learns the assembled-ep -> actual-points map from those
    predictions. Fitting on in-sample predictions would learn the components'
    training-set optimism instead of the bias a live run actually has.
    ...
    A frame with a single season leaves nothing to hold out, so the result is
    an unfitted (identity) model.
    """
