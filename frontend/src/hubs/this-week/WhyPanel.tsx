...
import {
  Callout, Card, PosBadge, TABLE_CLASS, THEAD_CLASS, fmtNum, fmtPct, tdClass,
  thClass,
} from '../../kit'
import type {
  AdviceDiff, ComponentPlayer, ComponentsBreakdown, OverridesPanel,
} from '../../types'
import AdviceDiffRows from './AdviceDiffRows'
...
function DiffStrip({ diff }: { diff: AdviceDiff }) {
  return (
    <Callout className="mb-4">
      ...
      {/* v19e §2.2: the sentence itself is shared with the History tab's
          comparison, because both are the same question about the same
          payload and one of them would otherwise drift. */}
      <AdviceDiffRows diff={diff} />
    </Callout>
  )
}
